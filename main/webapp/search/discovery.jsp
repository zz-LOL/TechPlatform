...
<fmt:message key="jsp.search.title"/>
</font>
...
%>                                </select><br/>
                                ...
                                <input type="submit" id="main-query-submit" class="btn2 btn-primary2" style="font-size:18px;width:53px;height:26px" value="<fmt:message key="jsp.general.go"/>" />
<% if (StringUtils.isNotBlank(spellCheckQuery)) {%>
	<p class="lead" style="display:none;"><fmt:message key="jsp.search.didyoumean"><fmt:param><a id="spellCheckQuery" data-spell="<%= Utils.addEntities(spellCheckQuery) %>" href="#"><%= spellCheckQuery %></a></fmt:param></fmt:message></p>
<% } %>                  
...
<% if (appliedFilters.size() > 0 ) { %>                                
		...
		<%
			...
			for (String[] filter : appliedFilters)
			{
				...
				<input class="btn1 btn-primary2" type="submit" id="submit_filter_remove_<%=idx %>" name="submit_filter_remove_<%=idx %>" value="X" />
				<br/>
				<%
				idx++;
			}
...
<% } %>
<a class="btn btn-default" style="display:none;" href="<%= request.getContextPath()+"/simple-search" %>"><fmt:message key="jsp.search.general.new-search" /></a>	
		</form>
		</div>
...
<% } %>
   ...
   <div class="discovery-pagination-controls panel-footer" style="display:none;">
   <form action="simple-search" method="get">
   <input type="hidden" value="<%= Utils.addEntities(searchScope) %>" name="location" />
   <input type="hidden" value="<%= Utils.addEntities(query) %>" name="query" />
	<% if (appliedFilterQueries.size() > 0 ) { 
...
%>
	...
	<div class=""><fmt:message key="jsp.search.results.results">
...
<%
	for (DiscoverySearchFilterFacet facetConf : facetsConf)
	{
	    ...
	    %><div id="facet_<%= f %>" class="panel " style="border:1px solid #9bc2e5">
                <div class="panel-heading" style="margin-left:-5px;"><img src="<%=request.getContextPath()%>/image/232.png">&nbsp;&nbsp;<fmt:message key="<%= fkey %>" /></div>
	    <ul class="list-group"><%
	    int idx = 1;
	    ...
	    {
	        %><li class="list-group-item2"><span style="visibility: hidden;">.</span>
	        <% if (currFp > 0) { %>
