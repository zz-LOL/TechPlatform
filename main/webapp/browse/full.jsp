...
<script type='text/javascript' src="<%= request.getContextPath() %>/static/js/jquery/jquery-1.10.2.min.js"></script>
    <script>
        $(function(){
            $(".evenRowOddCol>a").attr("target","_blank");
            $(".evenRowOddCol>strong>a").attr("target","_blank");
            $(".oddRowOddCol>strong>a").attr("target","_blank");
            $(".oddRowOddCol>a").attr("target","_blank");
        });
    </script>
<dspace:layout style="popup" titlekey="browse.page-title" navbar="<%=layoutNavbar %>">

	<%-- Build the header (careful use of spacing) --%>
	<!--<font style="font-family:STHeiti;font-size:36px;font-weight:bold;">
	...
	<div id="browse_navigation" class="well1 text-center" style="min-height:20px; padding:10px;">
...
<%
	...
	{
%>
		<span style="font-family:STHeiti;font-weight:bold;"><fmt:message key="browse.nav.date.jump"/> </span>
		<select name="year">
	        <option selected="selected" value="-1" style="font-family:STHeiti;font-weight:bold;"><fmt:message key="browse.nav.year"/></option>
<%
...
%>
        ...
        <select name="month">
            <option selected="selected" value="-1" style="font-family:STHeiti;font-weight:bold;"><fmt:message key="browse.nav.month"/></option>
<%
		for (int i = 1; i <= 12; i++)
		{
%>
...
%>
        ...
        <label for="starts_with"  style="font-family:STHeiti;font-weight:bold;"><fmt:message key="browse.nav.type-year"/></label>
        <input type="text" name="starts_with" size="4" maxlength="4"/>
        <input type="submit" class="btn2 btn-primary2" style="font-size:18px;margin-left:44px;min-width:53px;min-height:26px" value="<fmt:message key="browse.nav.go"/>" />
        <br/>
...
<%
	...
	{
%>	
		<span style="font-family:STHeiti;font-weight:bold;"><fmt:message key="browse.nav.jump"/></span>
	                        <a class="badge" href="<%= sharedLink %>&amp;starts_with=0">0-9</a>
<%
	    for (char c = 'A'; c <= 'Z'; c++)
	    {
%>
	                        <a class="badge" href="<%= sharedLink %>&amp;starts_with=<%= c %>"><%= c %></a>
<%
	    }
%><br/><br/><br/>
	    					<label for="starts_with" style="margin-left:-113px;margin-top:-2px;position:absolute;"><font style="font-size: 20px;font-weight:bold; margin-top:"><fmt:message key="browse.nav.enter"/></font></label>
		<input type="text" style="width:300px;margin-left:27px;" name="starts_with"/>
		<input type="submit" class="btn2 btn-primary2" style="font-size:18px;margin-right:44px;min-width:53px;min-height:26px" value="<fmt:message key="browse.nav.go"/>" />
<%
	}
%>
	</form>
	...
	<div id="browse_controls" class="text-center"  style="margin-bottom:30px;">
	<form method="get" action="<%= formaction %>">
		<input type="hidden" name="type" value="<%= bix.getName() %>"/>
<%
		if (bi.hasAuthority())
...
%>
		...
		<input type="submit" class="btn1 btn-primary2" style="font-size:16px;min-width:53px;min-height:26px" name="submit_browse" value="<fmt:message key="jsp.general.update"/>"/>
...
<div class="panel " style="overflow: hidden;background-color:#f8f8f8;border:1px solid #cfcfcf;">
	<%-- give us the top report on what we are looking at --%>
	<div class="panel-heading1 text-center">
