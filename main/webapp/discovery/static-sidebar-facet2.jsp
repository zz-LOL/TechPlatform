...
<!--<h3 class="facets"><fmt:message key="jsp.search.facet.refine" /></h3>-->
         ...
         <div id="onemore" class="pull-right" style="width:100%;">
<%
	for (DiscoverySearchFilterFacet facetConf : facetsConf)
	{
    	String f = facetConf.getIndexFieldName();
    	List<FacetResult> facet = mapFacetes.get(f);
		if(facet!=null){ facet=facet.subList(0, 8);}
 	    if (facet == null)
 	    {
 	        facet = mapFacetes.get(f+".year");
 	    }
        ...
        <div id="facet_<%= f %>" class="jpg" style="width:100%;border-bottom: dashed 1px #3CC;">
	    <div class="panel-heading" style="margin-top:15px;"><img src="<%=request.getContextPath()%>/image/232.png" style="margin-left:-10px;">&nbsp&nbsp<fmt:message key="<%= fkey %>" /></div>
	    <ul class="list-group" ><%
	    int idx = 1;
	    ...
	    if (facet != null)
	    {
		    ...
		    { 
		        if (idx != limit)
		        {
		        %><li class="list-group-item1"><span class="badge pull-right"><%= fvalue.getCount() %></span> <a href="<%= request.getContextPath()
		            + searchScope
